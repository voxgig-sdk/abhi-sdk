...
The Golang SDK for the Abhi API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/abhi-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/abhi-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/abhi-sdk/go=../abhi-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *AbhiSDK
```
...
| `Logo` | `(data map[string]any) AbhiEntity` | Create a Logo entity instance. |
| `Tool` | `(data map[string]any) AbhiEntity` | Create a Tool entity instance. |

### Entity interface (AbhiEntity)

All entities implement the `AbhiEntity` interface.
...
Create an instance: `download := client.Download(nil)`
...
```go
download, err := client.Download(nil).Load(nil, nil)
if err != nil {
    panic(err)
}
...
Create an instance: `fun := client.Fun(nil)`
...
```go
fun, err := client.Fun(nil).Load(nil, nil)
if err != nil {
    panic(err)
}
...
Create an instance: `logo := client.Logo(nil)`
...
```
github.com/voxgig-sdk/abhi-sdk/go/
├── abhi.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/abhi-sdk/go`) re-exports everything needed

...
class AbhiMakeError
{
    public static function call(?AbhiContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new AbhiContext([], null);
        }
        $op = $ctx->op ?? new AbhiOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new AbhiResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof AbhiError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new AbhiError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof AbhiError) {

...
class FtCtrl extends AbhiControl
{
...
class FtClient
{
    ...
    private AbhiUtility $utility;

    public function __construct(AbhiUtility $utility, array $options)
    {
    ...
    public function get_utility(): AbhiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AbhiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new AbhiUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (AbhiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AbhiConfig::make_feature($name);
    ...
    private static function buildUrl(AbhiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AbhiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AbhiResponse($fetched);
        }
    ...
    private function fail(AbhiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AbhiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

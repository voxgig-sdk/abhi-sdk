...
describe('ToolEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'tool.' + op, live)) return
    }
    ...
    let tool_ref01_data = setup.data.new.tool['tool_ref01']

    tool_ref01_data = await tool_ref01_ent.create(tool_ref01_data)
    assert(null != tool_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['ABHI_TEST_TOOL_ENTID']

...
//     (\`new AbhiSDK(...)\` / \`AbhiSDK.test(...)\` become
//     \`AbhiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"anime":{"test01":{"id":"test01"}},"download":{"test01":{"id":"test01"}},"fun":{"test01":{"id":"test01"}},"game":{"test01":{"id":"test01"}},"logo":{"test01":{"id":"test01"}},"tool":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AbhiSDK(...)\` and any
// \`AbhiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AbhiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AbhiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

...
The TypeScript SDK for the Abhi API — a type-safe, entity-oriented client with full async/await support.
...
| `tester(testopts?, sdkopts?)` | `AbhiSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `AbhiSDK.test(testopts?, sdkopts?)` | `AbhiSDK` | Create a test-mode client. |
...
| `client` | `client(): AbhiSDK` | Return the parent SDK client. |
...
```ts
const fun = await client.Fun().load()
```

// Command Abhi-cli is a CLI + REPL for the Abhi SDK,
...
const entitiesHelp = "anime download fun game logo tool"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ABHI_APIKEY (api key), ABHI_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ABHI_APIKEY carries the API key and
	// ABHI_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ABHI_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ABHI_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAbhiSDK(opts)

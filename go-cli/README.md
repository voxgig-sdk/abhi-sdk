...
AQL-driven command-line client **and** interactive REPL for the Abhi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/abhi-cli)
make build
...
./abhi-cli load 1 anime            # {id:1} shorthand
./abhi-cli load '{id:1}' anime       # explicit match map

# 4. Override the API base URL for a single call
ABHI_BASE=https://api.example.com ./abhi-cli load 1 anime
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/abhi-cli
   ```
...
```sh
./abhi-cli load 1 anime          # scalar shorthand for {id:1}
./abhi-cli load '{id:1}' anime     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `abhi>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/abhi-cli`. |
...
(`./abhi-cli <expr>`) and interactively (the REPL), and expressions compose the

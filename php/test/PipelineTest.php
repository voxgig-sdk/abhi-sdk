...
{
    private static function utility(): AbhiUtility
    {
        return new AbhiUtility();
    }
    ...
    private static function ctx(array $over = []): AbhiContext
    {
        ...
        $ctx->op = new AbhiOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AbhiError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AbhiOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AbhiMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AbhiOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AbhiMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AbhiOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AbhiMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AbhiSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AbhiMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AbhiResponse([]),
            'result' => new AbhiResult([]),
        ]);
        [, $err] = AbhiMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AbhiSpec(['step' => 's']),
            'response' => new AbhiResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AbhiResult([]),
        ]);
        [, $err] = AbhiMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AbhiSpec(['step' => 's']),
            'response' => new AbhiResponse(self::resp(200, ['v' => 1])),
            'result' => new AbhiResult([]),
        ]);
        [, $err] = AbhiMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AbhiSpec(['step' => 's']),
            'response' => new AbhiResponse(self::resp(200, ['v' => 2])),
            'result' => new AbhiResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AbhiMakeResponse::call($ctx);
    ...
    {
        $preset = new AbhiResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AbhiSpec([]),
            'response' => new AbhiResponse([]),
            'result' => new AbhiResult([]),
        ...
        [$got, $err] = AbhiMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AbhiResult([])]);
        [, $err] = AbhiMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AbhiSpec([]), 'result' => null]);
        [, $err] = AbhiMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AbhiSpec(['step' => 's']),
            'result' => new AbhiResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AbhiOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AbhiMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AbhiSpec(['step' => 's']),
            'result' => new AbhiResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AbhiOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AbhiMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AbhiResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AbhiSpec([]),
            'result' => new AbhiResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AbhiMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new AbhiError('boom', 'boom');
        $utility->fetcher = function (AbhiContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AbhiSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AbhiMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AbhiContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AbhiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AbhiResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AbhiContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AbhiSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AbhiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AbhiResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AbhiSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AbhiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AbhiMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AbhiResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AbhiDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AbhiResult(['ok' => false])]);
        $this->expectException(AbhiError::class);
        AbhiDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AbhiResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AbhiMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AbhiBaseFeature();
        $b = new AbhiBaseFeature();
        AbhiFeatureAdd::call($ctx, $a);
        AbhiFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AbhiBaseFeature
    {
        $f = new AbhiBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): AbhiContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new AbhiSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new AbhiResponse(['headers' => null]),
            'result' => new AbhiResult([]),
        ]);
        AbhiResultHeaders::call($ctx);

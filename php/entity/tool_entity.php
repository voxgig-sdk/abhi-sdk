...
class ToolEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ToolEntity($this->_client, $opts);
    }

    /**
     * @param Tool|array $args Tool data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = AbhiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Tool|array The current Tool data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Tool fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = AbhiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Tool fields).
     */
     ...
     * Load a single Tool.
     *
     * @param ToolLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ToolLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Tool|array The loaded Tool as an assoc-array at the
     *   SDK boundary; throws AbhiError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = AbhiHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     * Create a new Tool.
     *
     * @param ToolCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed ToolCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Tool|array The created Tool as an assoc-array at the
     *   SDK boundary; throws AbhiError on failure (item-5 convention).
     */

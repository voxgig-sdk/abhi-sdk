...
class DownloadEntity
  ...
  def make
    opts = @_entopts.dup
    DownloadEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AbhiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Download, Hash] the current Download data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AbhiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Download fields)
  def match_get
  ...
  #
  # @param reqmatch [DownloadLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Download.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Download, Hash] the loaded Download; raises AbhiError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AbhiHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end

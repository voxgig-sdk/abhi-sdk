...
local DownloadEntity = {}
DownloadEntity.__index = DownloadEntity


function DownloadEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DownloadEntity)
  self._name = "download"
  self._client = client
...
function DownloadEntity:get_name()
  return self._name
end


function DownloadEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DownloadEntity.new(self._client, opts)
end


function DownloadEntity:data_set(args)
  if args ~= nil then
...
function DownloadEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DownloadEntity:match_set(args)
  if args ~= nil then
...
function DownloadEntity:match_get()
...
function DownloadEntity:stream(action, args, callopts)
...
---@param reqmatch DownloadLoadMatch
---@param ctrl? table
---@return Download
---@return string? err
function DownloadEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DownloadEntity:_run_op(ctx, post_done)
  local utility = self._utility

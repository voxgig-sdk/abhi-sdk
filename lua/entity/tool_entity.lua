...
local ToolEntity = {}
ToolEntity.__index = ToolEntity


function ToolEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ToolEntity)
  self._name = "tool"
  self._client = client
...
function ToolEntity:get_name()
  return self._name
end


function ToolEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ToolEntity.new(self._client, opts)
end


function ToolEntity:data_set(args)
  if args ~= nil then
...
function ToolEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ToolEntity:match_set(args)
  if args ~= nil then
...
function ToolEntity:stream(action, args, callopts)
...
function ToolEntity:_run_op(ctx, post_done)
  local utility = self._utility

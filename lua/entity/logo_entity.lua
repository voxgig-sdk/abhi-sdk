...
local LogoEntity = {}
LogoEntity.__index = LogoEntity


function LogoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LogoEntity)
  self._name = "logo"
  self._client = client
...
function LogoEntity:get_name()
  return self._name
end


function LogoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LogoEntity.new(self._client, opts)
end


function LogoEntity:data_set(args)
  if args ~= nil then
...
function LogoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LogoEntity:match_set(args)
  if args ~= nil then
...
function LogoEntity:stream(action, args, callopts)
...
function LogoEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
The Lua SDK for the Abhi API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Anime(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Abhi(nil):load(
  { id = "test01" }, nil
)
...
The main module (`abhi_sdk`) exports the SDK constructor

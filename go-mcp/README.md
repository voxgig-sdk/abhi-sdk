...
MCP server exposing the Abhi SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `abhi_list` | `entity`, optional `query` map | First page of records as JSON |
| `abhi_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `abhi_list` and
`abhi_load` tools become available in new sessions.

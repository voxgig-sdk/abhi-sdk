...
[MCP](https://modelcontextprotocol.io) server exposing the Abhi SDK as
two agent tools — `abhi_list` and `abhi_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/abhi-mcp)
make build

# 2. Provide credentials via the environment
export ABHI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user abhi \
  -- /absolute/path/to/abhi-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./abhi-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// abhi_list: first page of records
{ "entity": "game" }
{ "entity": "game", "query": { } }

// abhi_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/abhi-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user abhi \
     -- "$PWD"/dist/*/abhi-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `abhi_list` and `abhi_load` tools now appear
   in new sessions. Ask the agent to *"list game using abhi"*
   and it calls `abhi_list` with `{"entity":"game"}`.
...
```sh
export ABHI_APIKEY=sk_live_xxx            # API key
export ABHI_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `abhi_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `abhi_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ABHI_APIKEY` | API key sent with every request. |
| `ABHI_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

anime | download | fun | game | logo | tool

### Smoke test via HTTP (raw JSON-RPC)

```sh
./abhi-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"abhi_load","arguments":{"entity":"anime","query":{"id":1}}}}'
```
